...
- `PROSTAR_ENROLLMENT_SECRET`: 32–256 URL-safe characters; signs short-lived
  enrollment claims. Each Mac generates its own credential locally.
...
4. On macOS 15 or later, let Prostar install its private runtime; no Homebrew,
   Node.js, developer tools, or administrator access is required.
5. Approve macOS Screen Recording access if requested.
6. Wait for the sole success line: `Prostar installed successfully.`
7. Return to the dashboard; the Mac appears under **Sessions**.
...
reinstall on a Mac already paired with this dashboard securely reuses that
Mac's existing scoped identity, so it does not create a duplicate session.
